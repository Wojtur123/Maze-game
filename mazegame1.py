...
class Game:
    def __init__(self, name_of_gamefile):
        ...
        with open(name_of_gamefile, 'r') as gamefile:
            y = yaml.load(gamefile)
            self.player_position = y['player_position']
            self.board = y['board']
    ...
    def getinput(self):
        while True:
            tempinput = input("Escape from the maze !\nUse W S A D to move. Good luck !")
...
while True:
    ...
    z = input('To load game press l, to new game press n')
    if z in 'lL':
        game = Game('save.yml')
    else:
        game = Game('board.yml')
